...
app.post("/api/booking", upload.single("idCard"), async (req, res) => {
  try {
    ...
    // 📤 Upload to FTP only if not alumni
    if (userType !== 'alumni') {
      if (!req.file) {
        return res.status(400).json({ error: "ID card file is required for this user type" });
      }
      const remoteFilename = `${email}.jpg`;
      ftpPath = await uploadToFTP(req.file.buffer, remoteFilename);
    }
...
// 📥 Booking Endpoint
app.post("/api/booking1", upload.single("idCard"), async (req, res) => {
  try {
    const {
      name,
      rollNo,
